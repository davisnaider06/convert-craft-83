...
// Ensure THREE is available globally for external UMD scripts
...
export const GridScan: React.FC<GridScanProps> = ({
  ...
  useEffect(() => {
    ...
    if (enablePost && (window as any).POSTPROCESSING) {
      const PP = (window as any).POSTPROCESSING;
      ...
      const effectPass = new PP.EffectPass(camera, bloom, chroma);
  ...
  useEffect(() => {
    if (!libsReady || !(window as any).faceapi) return;
  ...
  }, [libsReady, enableWebcam, modelsReady, depthResponse]);
